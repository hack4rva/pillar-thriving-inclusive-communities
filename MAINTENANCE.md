...
**Thriving and Inclusive Communities Pillar Research Corpus**
...
If the report spans sections or introduces a new cross-cutting topic, use the next available number in the `94–98` range (cross-cutting; 90–93 are already used).

### Step 2 — Name the file consistently

Pattern: `[SECTION][NUMBER]_[topic_slug].md`

Example: `A6_problem_landscape_digital_exclusion.md`

Use lowercase snake_case for the topic slug. Keep it short but descriptive. Check that no file with that name already exists in `research/`.
...
source: perplexity-sonar-deep-research | parallel-ai | manual | other
...
**Privacy note:** If the report contains any claims about data collection or PII handling, add:
```yaml
...
Add a new entry to the JSON array in the correct section order:

```json
{
  "id": "A6_problem_landscape_digital_exclusion",
  "section": "A",
  "title": "Your Report Title",
...
Rules:
- `id` must match the filename exactly (without `.md`)
- `summary` must reflect the actual file content, not an invented description
- `key_terms` should be terms a reader would search for to find this file
- Insert in the correct section order (not appended to end)
- Validate after editing: `python3 -m json.tool research/index.json`

### Step 5 — Update `research/INDEX.md`

Add a row to the appropriate section in `research/INDEX.md`:

```markdown
- [`A6_problem_landscape_digital_exclusion.md`](A6_problem_landscape_digital_exclusion.md) — Brief description of what this report covers
```

### Step 6 — Update `manifest.json`

Add an entry to the root manifest for the new file:

```json
{
  "id": "research/A6_problem_landscape_digital_exclusion",
  "path": "research/A6_problem_landscape_digital_exclusion.md",
  "type": "research_report",
  "title": "Your Report Title",
  "pillar": "thriving-inclusive-communities",
  "section": "A",
  "summary": "One to two sentence grounded summary.",
  "tags": ["digital exclusion", "accessibility", "problem landscape"],
  "recommended_audience": ["teams exploring digital equity", "teams researching access barriers"],
  "read_after": ["research/A1_problem_landscape_immigrant_service_access.md"],
...
[ ] H1 heading in the file is accurate
[ ] frontmatter title matches H1 heading and index.json title
...
[ ] evidence_log.md updated if new verified claims are added
...
2. Update every reference: `research/INDEX.md`, `research/index.json`, `manifest.json`, `AGENTS.md`, `CORPUS_GUIDE.md`, any skill files, any report frontmatter with `related_reports`
3. Rename the file
...
`evidence_log.md` (root) tracks verified factual claims and documented information gaps.

When you verify a new claim:
1. Add an entry under `## Confirmed` or `## Likely but unverified` with a new `E-` ID
...
When you identify a new gap or missing dataset:
1. Add to the `## Missing` section with a new `E-M` ID
2. Describe what data would be needed and why it is not available

If a claim is found to be incorrect, update the entry with a correction note rather than deleting it.
...
The index.json should contain one entry per research `.md` file in `research/` (excluding `INDEX.md`). To regenerate:
1. List all `.md` files in `research/` (excluding INDEX.md)
2. For each file, extract: id (filename without extension), section (letter or label prefix), title (from H1 heading or frontmatter), summary (from frontmatter or write a new grounded one), key_terms (from frontmatter or extract from content)
...
3. Reference it from `README.md` Repo Map section if it is a primary user-facing template
...
[ ] research/INDEX.md rows include new files
[ ] evidence_log.md updated if new verified claims or gaps identified
[ ] No broken internal links (spot-check key references)
[ ] Frontmatter is accurate (no fabricated fields)
[ ] Privacy-sensitive content reviewed against community_trust_framework.md
...
| `evidence_log` | `evidence_log.md` | Verified claims and gaps | Yes |
